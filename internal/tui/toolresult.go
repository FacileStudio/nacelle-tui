...
func (m *Model) printGroupFailure(line, name string, errs []toolError, dur string) {
	styled := toolview.ColorGlyph(line, "31", toolview.ToolRestore(name)) + " · " + dur
...
func (m *Model) flushFailures() {
	...
	toolStr := m.paint(fromTool, toolview.ColorGlyph(m.run.failures.toolLine, "31", toolview.ToolRestore(m.run.failures.name)))
	var errLine string
	...
	m.unprinted = append(m.unprinted, toolStr+"\n"+resultStr)
	m.session.Line(sessions.Speaker(fromTool), toolview.ColorGlyph(m.run.failures.toolLine, "31", toolview.ToolRestore(m.run.failures.name)))
