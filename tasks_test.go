...
func TestAnUnknownStatusIsRefusedByName(t *testing.T) {
	_, err := tasksTool{}.Run(t.Context(), json.RawMessage(`{"tasks":[{"title":"a","status":"blocked"}]}`))
	if err == nil {
		t.Fatal("an unknown status was accepted")
	}
	if !strings.Contains(err.Error(), "blocked") || !strings.Contains(err.Error(), statusActive) {
...
// mislabelled one sees it in the reply instead of on the next call.
